...
const startSession = assign<TimerMachineContext, TimerEventStart>(
  (_, event) => ({
    startDate: new Date(),
...
const setSecondRemaining = assign<TimerMachineContext, TimerEventTick>(
  (context) => {
      ...
      on: {
        ...
        [TimerMachineEventType.UNPAUSE]: {
          ...
          actions: [
            assign((ctx, ev) => ({
              pauseBuffer:
                ctx.pauseBuffer + (Date.now() - ctx.pauseDate.getTime()) / 1000,
            })),
            sendParent((ctx, ev) => ({
              ...ev,
              type: "RECORD_PAUSE",
